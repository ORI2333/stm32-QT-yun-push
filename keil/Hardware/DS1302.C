...
void DS1302_GPIOInit(void)
{
  ...
  GPIO_InitStructre.GPIO_Pin= GPIO_Pin_10|GPIO_Pin_11|GPIO_Pin_12;
...
void DS1302_IO_GPIO(uchar FLAG)
{
  ...
  //RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB,ENABLE);            /*open GPIO  clock*/
    ...
    if(FLAG==0x01)
  {
    GPIO_InitStructre.GPIO_Pin= GPIO_Pin_11;//配置IO_GPIO
  GPIO_InitStructre.GPIO_Speed=GPIO_Speed_50MHz;
  GPIO_InitStructre.GPIO_Mode=GPIO_Mode_Out_PP;
  GPIO_Init(DS1302_PORT, &GPIO_InitStructre);
  }
...
//    label2[15]=DS1302_Read( 0x81)%10;     //读秒
		...
		Time[0] = DS1302_Read( 0x8D);//年
		Time[1] = DS1302_Read( 0x89);//月
		Time[2] = DS1302_Read( 0x87);//日
		Time[3] = DS1302_Read( 0x85);//时
		Time[4] = DS1302_Read( 0x83);//分
		Time[5] = DS1302_Read( 0x81);//秒
}
